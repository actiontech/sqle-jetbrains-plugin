...
public class InstStatusBarWidget extends EditorBasedStatusBarPopup implements Observer {
    ...
    protected @Nullable ListPopup createPopup(DataContext context) {
        ...
        try {
            String projectID = SQLESettingUI.projects.get(SQLESettingUI.settings.getProjectName());
            instList = client.GetDataSourceNameList(projectID, SQLESettingUI.settings.getDBType());
        } catch (Exception e) {
